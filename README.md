...
Интеграция создаёт устройство `Сценарии` с календарём `Временные сценарии`.
Календарь показывает отложенные действия, созданные командами вроде «закрой шторы через 10 минут» или «включи свет на час», и позволяет отменять их. Событие длится от момента создания временного сценария до запланированного выполнения.

Для устройств, добавленных в Яндекс через Home Assistant, календарь показывает `entity_id` в описании события и комнату в поле места. Действие отображается в названии события, например `Выключить Люстра`.

При создании события укажите действие для Алисы в названии. Время начала превращается в задержку, а длительность больше одной минуты — в команду «на …». В поле места можно указать точное имя станции; если оно не заполнено, используется первая станция аккаунта.
